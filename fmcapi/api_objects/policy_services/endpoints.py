...
class Endpoints(APIClassTemplate):
    ...
    def vpn_policy(self, pol_name):
        ...
        if 'id' in ftd_s2s.__dict__:
            ...
            self.URL = f'{self.fmc.configuration_url}{self.PREFIX_URL}/{self.vpn_id}/endpoints'
    ...
    def endpoint(self, action, device_name):
        ...
        if action == 'add':
            ...
            if new_device is None:
                logging.warning(f'Device/DeviceHA "{device_name}" is not found in FMC.  Cannot add to Endpoints.')
            else:
            ...
            if 'device' in self.__dict__:
                ...
                logging.warning(
                    'Endpoints has no members.  Cannot remove device.')
        elif action == 'clear':
            if 'device' in self.__dict__:
                del self.device
    ...
    def vpn_interface(self, device_name, ifname):
        ...
        ether_json = EtherchannelInterfaces(
            fmc=self.fmc, device_name=device_name).get()
        phys_json = PhysicalInterfaces(
            fmc=self.fmc, device_name=device_name).get()
        redund_json = RedundantInterfaces(
            fmc=self.fmc, device_name=device_name).get()
        subintf_json = SubInterfaces(
            fmc=self.fmc, device_name=device_name).get()
        items = ether_json.get('items', []) + phys_json.get('items', []) + \
            redund_json.get('items', []) + subintf_json.get('items', [])
        new_intf = None
        for item in items:
            if item["ifname"] == ifname:
                ...
                break
        if new_intf is None:
            logging.warning(f'Interface "{ifname}" is not found in FMC.  Cannot add to interface.')
        else:
