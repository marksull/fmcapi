...
class IPSecSettings(APIClassTemplate):
	...
	VALID_JSON_DATA = ['id', 'name', 'type', 'tfcPackets', 'validateIncomingIcmpErrorMessage', 'enableSaStrengthEnforcement', 'perfectForwardSecrecy',
	    'lifetimeSeconds', 'doNotFragmentPolicy', 'lifetimeKilobytes', 'cryptoMapType', 'ikeV2Mode', 'enableRRI', 'ikeV2IpsecProposal', 'ikeV1IpsecProposal']
	...
	def vpn_policy(self, pol_name):
		...
		if 'id' in ftd_s2s.__dict__:
			...
			logging.warning(f'FTD S2S VPN Policy "{name}" not found.  Cannot set up IPSecSettings for FTDS2SVPNs Policy.')
	...
	def ipsec_policy(self, pol_name, version=1, action="add"):
		...
		if action == 'add':
			...
			if new_pol is None:
				logging.warning(f'IKEv"{version}"IpsecProposal "{pol_name}" not found.  Cannot set up IPSecSettings Policy.')
			else:
				if 'version' == 1:
